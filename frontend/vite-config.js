...
const PREVIEW_HOST_ENV = "app_PREVIEW_HOST";
...
* app preview proxy so assets resolve when the page is loaded directly
* from the sandbox dev server (e.g. `localhost:8080`) instead of via
* `*.app.app` / `*.appproject.com`.
*
* No-op unless `app_PREVIEW_HOST` is set (host only, no scheme), e.g.
* `id-preview--<project-id>.app.app`.
*/
function appAssetsProxyPlugin() {
	const previewHost = process.env[PREVIEW_HOST_ENV]?.trim();
	return {
		name: "app:assets-proxy",
		apply: "serve",
		configureServer(server) {
			...
			const middleware = (req, res, next) => {
				...
				upstream.on("error", (err) => {
					server.config.logger.warn(`[@app.dev/vite-tanstack-config] asset proxy ${url} → ${previewHost} failed: ${err.message}`);
					if (!res.headersSent) {
						res.statusCode = 502;
						res.end();
...
const app_SANDBOX_ENV_VAR = "app_SANDBOX";
function isSandboxEnvironment() {
	return process.env[app_SANDBOX_ENV_VAR] === "1" || !!process.env[SANDBOX_ENV_VAR];
}
...
function logWarning(message) {
	console.warn(`[@app.dev/vite-tanstack-config] ${message}`);
}
...
const SSR_CAPTURE_KEY = "__app_TANSTACK_CAPTURE_SSR_ERROR__";
const HANDLER_RAN_KEY = "__app_TANSTACK_HANDLER_RAN__";
...
const BUILD_DIAGNOSTICS_PLUGIN_NAME = "app-build-error-diagnostics";
...
function devSsrErrorLogger() {
	...
	const handlerRanFlag = Symbol("appHandlerRan");
	const capture = (error) => {
		lastCapture = {
			error,
			at: Date.now()
			...
			server.middlewares.use((req, res, next) => {
				...
				const handleErrorResponse = () => {
					...
					if (!res.headersSent) try {
						res.setHeader("x-app-error-source", source);
					} catch {}
...
function defineConfig(configOrOptions = {}) {
	return async (env) => {
		...
		if (command === "build" && isSandbox) internalPlugins.push(appBuildErrorDiagnostics());
		...
		if (options.nitro !== false && command === "build") {
			...
			try {
				...
				if (explicitNitro || isSandbox) {
					const reason = explicitNitro ? `an explicit \`nitro\` option (${options.nitro === true ? "true" : "{ ... }"}) was set in vite.config.ts` : "running inside a app sandbox";
					throw new Error(`[@app.dev/vite-tanstack-config] ${reason}, but the \`nitro\` package isn't installed. Add \`nitro\` to your devDependencies (matches peerDependency >=3.0.260429-beta). Original import error: ${err instanceof Error ? err.message : String(err)}`);
				}
			}
			if (nitroMod) {
		...
		if (command === "serve") {
			...
			if (isSandbox ? hmrGateOpt !== false : !!hmrGateOpt) {
				try {
					const { hmrGatePlugin } = await import("@app.dev/vite-plugin-hmr-gate");
		...
		if (command === "serve" && isSandbox) {
			try {
				const { devServerBridgePlugin } = await import("@app.dev/vite-plugin-dev-server-bridge");
				internalPlugins.push(devServerBridgePlugin());
			} catch (e) {}
		}
		if (command === "serve") internalPlugins.push(appAssetsProxyPlugin());
		let envDefine = {};
...
//#endregion
export { defineConfig, appAssetsProxyPlugin };
